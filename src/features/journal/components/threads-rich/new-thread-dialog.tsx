...
const DEFAULT_ASSIGNEE = DEMO_USERS[0]
const EMPTY_HTML = "<p></p>"

export function RichNewThreadDialog({ open, onOpenChange }: NewThreadDialogProps) {
  ...
  const { createThread, loading } = useRichThreadsStore()
  const isSubmitting = loading.createThread

  const hasDescription = description.length > 0 && description !== EMPTY_HTML
  ...
  const handleSubmit = async () => {
    if (!(title.trim() && hasDescription) || isSubmitting) {
      return
    }

    await createThread(title.trim(), description, attachments, assignee)
    setTitle("")
    setDescription("")
    setAttachments([])
      ...
      <DialogContent className="sm:max-w-[560px]">
        ...
        <div className="space-y-4 py-2">
          <div className="flex flex-col gap-2">
            <label className="font-medium text-sm" htmlFor="rich-thread-title">
              Title
            </label>
            <Input
              disabled={isSubmitting}
              id="rich-thread-title"
          ...
          <div className="flex flex-col gap-2">
            <label className="font-medium text-sm" htmlFor="rich-thread-description">
              Description
            </label>
            <RichTextEditor
              content={description}
        ...
        <DialogFooter>
          ...
          <Button
            disabled={!(title.trim() && hasDescription) || isSubmitting}
